...
export default function Home() {
  ...
  const handleBuyWithBNB = async () => {
    ...
    try {
      ...
      const tokenAddress = await presaleContract.tokenAddress();

      const tokenAbi = ["function balanceOf(address) view returns (uint256)"];

      const tokenContract = new ethers.Contract(tokenAddress, tokenAbi, signer);

      const tokenBalanceInPresale = await tokenContract.balanceOf(
        presaleContract.target
      );

      console.log(
        "Token balance in Presale (REAL):",
        Number(tokenBalanceInPresale) / 1e18
      );
